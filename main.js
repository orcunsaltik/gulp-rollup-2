            ...
            await Promise.all(bundles.map(async bundle => {
                ...
                await Promise.all(oo.map(async o => {
                    ...
                    if ((f === 'umd' || f === 'amd') && !o.amd || !!o.amd && !o.amd.id) {
                        o.amd = Object.assign({}, o.amd, {id: o.name});
                    }
...
const outside = async function (opts) {
    ...
    await Promise.all(bundles.map(async bundle => {
        ...
        await Promise.all(oo.map(async o => {
            ...
            const oName = path.basename(o.file);
            const oPath = path.resolve(oBase, oName);
            const oFile = new vinyl({ base: oBase, path: oPath, contents: buffer });
...
module.exports = { 'rollup': inside, 'src': outside };